    ...
    <div class="space-y-6">
        <div class="flex flex-wrap items-end gap-4 rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            <div class="ml-auto text-sm text-gray-500">
                Rekap kas kecil untuk {{ $this->monthOptions[$this->bulan] ?? $this->bulan }} {{ $this->tahun }}
            </div>
        </div>
        ...
        <div class="grid gap-4 md:grid-cols-3">
            <div class="rounded-xl border border-emerald-200 bg-emerald-50 p-4 shadow-sm dark:border-emerald-800 dark:bg-emerald-900/20">
                ...
                <div class="mt-2 text-2xl font-semibold text-emerald-800 dark:text-emerald-200">
                    Rp {{ number_format($report['total_pengisian'], 0, ',', '.') }}
                </div>
            </div>

            <div class="rounded-xl border border-rose-200 bg-rose-50 p-4 shadow-sm dark:border-rose-800 dark:bg-rose-900/20">
                <div class="text-sm text-rose-700 dark:text-rose-300">Total Pengeluaran</div>
            ...
            <div class="rounded-xl border border-sky-200 bg-sky-50 p-4 shadow-sm dark:border-sky-800 dark:bg-sky-900/20">
                ...
                <div class="mt-2 text-2xl font-semibold text-sky-800 dark:text-sky-200">
                    Rp {{ number_format($report['saldo'], 0, ',', '.') }}
                </div>
            </div>
        </div>
            ...
            <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="px-4 py-6 text-center text-gray-400">Belum ada data pivot.</td>
                            </tr>
                        @endforelse
                    </tbody>
                    @if ($report['pivot']->isNotEmpty())
                        <tfoot class="bg-amber-50 dark:bg-amber-900/20">
                            <tr>
                                <td colspan="2" class="px-4 py-3 font-semibold text-amber-800 dark:text-amber-200">Grand Total</td>
                                <td class="px-4 py-3 text-right font-bold text-amber-800 dark:text-amber-200">
                                    Rp {{ number_format($report['pivot']->sum('total'), 0, ',', '.') }}
                                </td>
                            </tr>
                        </tfoot>
                    @endif
